...
if (Platform.OS !== 'web') {
  TaskManager.defineTask<BackgroundLocationData>(ATIRA_BACKGROUND_LOCATION_TASK, async ({ data, error }) => {
    try {
      ...
      // Never log raw background payloads or database errors that may contain
      // sensitive values. This marker is enough for a connected-device console.
      console.error('[ATIRA_BACKGROUND_LOCATION:persistence-failed]');
    }
  });
}
